...
void printLevel(Node* root, int level, vector<int> &saved)
{
    ...
    if ( level == 0) {cout << root->data << " " ; saved.push_back(root->data);}
        ...
        printLevel(root->left, level-1,saved);
        printLevel(root->right, level-1, saved); 
    }
}
...
int main()
{
    ...
    printLevel(root, 2,saved); cout << endl;
    printLevel(root, 1,saved); cout << endl;
    printLevel(root, 0,saved); cout << endl;
    for(auto v : saved)
        cout << v << " ";
    cout << endl;
